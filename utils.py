...
class GraphData(data.Dataset):
    def __init__(self, path, train=False, test=True, seed=0):
        ...
        for i in tqdm(range(len(file_list))):
            # print("dir name {}".format(file_list[i]))
        ...
        for g in self.graphs:
            ...
            # print("node_features shape \n{}".format(g.node_features.shape))
            # print("node feature\n{}\n".format(g.node_features))
        ...
        graphs_num = len(self.graphs)
        # spilt the dataset as 7:3 between train dataset and test dataset.
        if self.test:
            self.graphs = self.graphs
        elif train:
            self.graphs = self.graphs[:int(0.7 * graphs_num)]
        else:
            self.graphs = self.graphs[int(0.7 * graphs_num):]
